...
document.addEventListener("DOMContentLoaded", () => {
    ...
    fetchAllDogs().then(dogs => {
        Object.keys(dogs).forEach(dog => {
            ...
            list.innerHTML = `<td>${dog}</td>
            <td><button type="button" onclick="onAddDog('${dog}')" class="btn btn-primary">Add</button>
            </td>`;
    ...
    fetchMyDogs().then(dogs => {
        dogs.forEach(dog => {
            let list = document.createElement('tr');
            list.innerHTML = `
            <td><input id="${dog.id + '-breed'}" class="form-control" type="text" value="${dog.breed}"></td>
            <td><input id="${dog.id + '-note'}" class="form-control" type="text" value="${dog.notes}"></td>
            <td><button type="button" onclick="updateMyDog(${dog.id})" class="btn btn-primary"> update </button></td>
            <td><button type="button" onclick="deleteMyDog(${dog.id})" class="btn btn-primary"> delete </button></td>
            `;
